...
import { Search, Filter, BookOpen, CheckCircle } from 'lucide-react-native';
...
export default function GrammarScreen() {
  ...
  const totalCount = useGrammarStore.getState().rules.length;

  const toggleFilters = () => {
    setShowFilters(!showFilters);
  };

  const renderGrammarCard = ({ item }) => (
    <GrammarCard
      title={item.title}
      explanation={item.explanation}
      examples={item.examples}
      level={item.level}
      onPress={() => markRuleCompleted(item.id)}
    />
  );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.progressContainer}>
          ...
          <Text style={styles.progressText}>
            {completedCount} of {totalCount} rules completed
          </Text>
          <ProgressBar 
      ...
      {filteredRules.length > 0 ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyTitle}>No grammar rules found</Text>
